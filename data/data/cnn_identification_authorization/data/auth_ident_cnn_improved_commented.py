...
SCRIPT_CONFIG = {
    ...
    # se quiser sobrescrever o caminho do dataset (manifest.csv ou joblib HOG histórico)
    "dataset_path_override": None,  # ex.: r"C:\\...\\celeba_rgb_128x128\\manifest.csv"

    # opcional: sobrescreve o nome do manifest (se o treino não usar "manifest.csv")
    "manifest_name_override": None,

    # camada usada para embeddings na autenticação:
    # - "auto": tenta ["dense", "gap", "flatten"] (e depois penúltima camada) – compatível com cnn_improved
    # - ou o nome exato de uma camada do modelo (ex.: "dense")
    "embedding_layer_name": "auto",
...
def mlp_forward_inference(
    ...
    if hasattr(modelo, "predict") and hasattr(modelo, "layers"):
        ...
        embedding_layer_name = str(inference_params.get("embedding_layer_name", "auto")).strip()

        # Resolve camada de embedding:
        # - "auto": tenta uma lista de nomes comuns (compatível com cnn_improved: dense/gap/flatten)
        # - caso contrário, usa o nome informado; se não existir, cai para penúltima camada.
        resolved_name = inference_params.get("_resolved_embedding_layer_name", None)
        if resolved_name is None:
            candidate_names = inference_params.get("embedding_layer_candidates", ["dense", "gap", "flatten"])
            if embedding_layer_name.lower() == "auto":
                picked = None
                for cand in candidate_names:
                    try:
                        modelo.get_layer(str(cand))
                        picked = str(cand)
                        break
                    except Exception:
                        continue
                resolved_name = picked if picked is not None else "__fallback_penultimate__"
            else:
                resolved_name = embedding_layer_name
            inference_params["_resolved_embedding_layer_name"] = resolved_name

        # model que devolve [probs, embedding] em 1 só forward
        combined = inference_params.get("_combined_model", None)
        if combined is None:
            try:
                if resolved_name != "__fallback_penultimate__":
                    emb_out = modelo.get_layer(resolved_name).output
                else:
                    raise KeyError("fallback")
            except Exception:
                # fallback: penúltima camada (tipicamente pré-softmax / pré-head)
        ...
        if X.ndim == 1 and X.dtype.kind in ("U", "S", "O"):
            ...
            assume_ingested_size = bool(inference_params.get("assume_ingested_size", False))
...
def main():
    ...
    if dataset_path:
        ...
        manifest_name = SCRIPT_CONFIG.get("manifest_name_override", None)
        if not manifest_name:
            manifest_name = str(cfg_train.get("MANIFEST_NAME", "manifest.csv")).strip() or "manifest.csv"
        dataset_path = (dataset_dir / manifest_name).resolve()
    ...
    k_folds = int(cfg_train.get("KFOLDS", cfg_train.get("K_FOLDS", 2)))
    ...
    inference_params = {
        ...
        "embedding_layer_name": str(SCRIPT_CONFIG.get("embedding_layer_name", "auto")),
        "embedding_layer_candidates": ["dense", "gap", "flatten"],
        "img_size": int(cfg_train.get("IMG_SIZE", 128)),
        "channels": int(cfg_train.get("IN_CHANNELS", 3)),
        "assume_ingested_size": bool(cfg_train.get("ASSUME_INGESTED_SIZE", False)),
