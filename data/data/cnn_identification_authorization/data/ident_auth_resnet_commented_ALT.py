...
def mlp_forward_inference(
    ...
    if hasattr(modelo, "predict") and hasattr(modelo, "layers"):
        ...
        if combined is None:
            ...
            outputs_logits = inference_params.get("outputs_logits", None)
            if outputs_logits is None:
                # heurística: se existir uma camada chamada "logits", assumimos que a saída é logits
                try:
                    modelo.get_layer("logits")
                    outputs_logits = True
                except Exception:
                    outputs_logits = False

            probs_out = modelo.output
            if bool(outputs_logits):
                probs_out = tf.keras.layers.Softmax(name="probs")(probs_out)

            combined = tf.keras.Model(inputs=modelo.input, outputs=[probs_out, emb_out])
...
def main():
    ...
    print(f"[INFO] Modelo ResNet carregado: {model_path}")
    ...
    # ------------------------------------------------------------
    # (Leakage guard) Preferir manifests explícitos salvos no treino
    # ------------------------------------------------------------
    holdout_manifest_path = out_dir / "final_test_manifest.csv"
    cv_manifest_path = out_dir / "cv_manifest.csv"

    use_explicit_holdout = bool(holdout_manifest_path.exists())
    dataset_path_eval = holdout_manifest_path if use_explicit_holdout else dataset_path
    # Para o conjunto de tuning, preferimos o pool do CV (sem holdout), se existir.
    if use_explicit_holdout and cv_manifest_path.exists():
        dataset_path_tune = cv_manifest_path
    else:
        dataset_path_tune = dataset_path
        if use_explicit_holdout and not cv_manifest_path.exists():
            print("[WARN] cv_manifest.csv não encontrado; tuning pode sofrer leakage. Recomendado re-treinar com o script atualizado.")

    if not dataset_path_eval.exists():
        raise FileNotFoundError(f"manifest.csv não encontrado: {dataset_path_eval}")

    if use_explicit_holdout:
        print(f"[INFO] Dataset (eval/holdout) = {dataset_path_eval}")
        print(f"[INFO] Dataset (tuning)       = {dataset_path_tune}")
    else:
        print(f"[INFO] Dataset (manifest) = {dataset_path_eval}")
    ...
    inference_params = {
        ...
        "outputs_logits": bool(SCRIPT_CONFIG.get("outputs_logits", False)),
    ...
    if use_explicit_holdout:
        # =========================================================
        # MODO SEM LEAKAGE (preferencial):
        # - X_eval vem do holdout explícito salvo no treino (final_test_manifest.csv)
        # - X_tune vem do pool do CV salvo no treino (cv_manifest.csv), quando disponível
        # =========================================================
        X_eval0, y_eval0, meta_eval = carregar_dataset_joblib(str(dataset_path_eval), label2idx=label2idx)
        p_eval = meta_eval.get("paths", None)
        id_eval = meta_eval.get("ids", None)
        if p_eval is not None:
            p_eval = np.asarray(p_eval)
        if id_eval is not None:
            id_eval = np.asarray(id_eval)

        # Se X_eval0 for caminho, apply_standardizer devolve o próprio X.
        X_eval_feat = apply_standardizer(X_eval0, norm_mean, norm_std)
        X_eval, y_eval, paths_eval, ids_eval, _ = filtrar_por_classes(
            X_eval_feat, y_eval0, classes_modelo, p_eval, id_eval, None
        )

        # Tuning: preferimos o pool do CV (sem holdout). Se não existir, caímos no manifest completo.
        X_tune0, y_tune0, meta_tune = carregar_dataset_joblib(str(dataset_path_tune), label2idx=label2idx)
        p_tune = meta_tune.get("paths", None)
        id_tune = meta_tune.get("ids", None)
        if p_tune is not None:
            p_tune = np.asarray(p_tune)
        if id_tune is not None:
            id_tune = np.asarray(id_tune)

        X_tune_feat = apply_standardizer(X_tune0, norm_mean, norm_std)
        X_tune, y_tune, paths_tune, ids_tune, _ = filtrar_por_classes(
            X_tune_feat, y_tune0, classes_modelo, p_tune, id_tune, None
        )
    else:
        # =========================================================
        # MODO LEGADO: reconstrói um split aproximado a partir do manifest.csv
        # (pode sofrer leakage se o treino usou outro holdout).
        # =========================================================
...
def _run_with_output_log(main_func, log_prefix="resnet_ident_auth_eval"):
...
if __name__ == "__main__":
     _run_with_output_log(main, log_prefix="resnet_ident_auth_eval")