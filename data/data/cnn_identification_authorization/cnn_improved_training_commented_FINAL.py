...
CONFIG: Dict[str, Any] = {
    ...
    # Augmentação leve de cor (sem JPEG quality) [Shorten19]
    "AUG_COLOR_JITTER": True,
    "AUG_BRIGHTNESS_DELTA": 0.10,      # random_brightness: delta em [0..1]
    ...
    # --------------------------------------------------------
    # Melhorias de alto impacto / baixo custo (configuráveis)
    # --------------------------------------------------------
    # (1) Padding "same" nas convoluções (melhora estabilidade espacial)
    "CNN_FORCE_SAME_PADDING": True,
    # (2) BatchNorm após convolução (acelera treino e estabiliza gradientes) [Ioffe15]
    "CNN_USE_BATCHNORM": True,
    # (3) GlobalAveragePooling2D no lugar de Flatten (reduz parâmetros e overfitting) [Lin13]
    "CNN_USE_GLOBAL_AVG_POOL": True,

    # (6) Label smoothing (regularização leve para classificação) [Szegedy16]
...
def _as_range2(v, name: str) -> Tuple[float, float]:
    """Aceita (a,b) float/list/tuple, ou string "0.9,1.1" e devolve (a,b)."""
    if v is None:
        return 1.0, 1.0
    if isinstance(v, (tuple, list)) and len(v) == 2:
    ...
    raise ValueError(f"Config inválida para {name}: {v!r} (esperado float ou par de floats)")
...
def build_cnn(cfg: Dict[str, Any], num_classes: int) -> Model:
    ...
    # Flags (pontos 1/2/3)
    force_same_padding = bool(cfg.get("CNN_FORCE_SAME_PADDING", False))
    use_batchnorm = bool(cfg.get("CNN_USE_BATCHNORM", False))
    use_gap = bool(cfg.get("CNN_USE_GLOBAL_AVG_POOL", False))

    if force_same_padding:
        conv1_padding_eff = "same"
        conv2_padding_eff = "same"
    else:
        conv1_padding_eff = conv1_padding
        conv2_padding_eff = conv2_padding

    # Conv block: Conv2D -> (BatchNorm) -> (Activation)
    def _conv2d_block(x, filters, kernel, strides, padding, act, base_name: str):
        # Quando usamos BatchNorm, a prática comum é remover bias da conv (redundante) [Ioffe15].
        x = layers.Conv2D(
            filters, kernel, strides=strides, padding=padding,
            activation=None if use_batchnorm else act,
            use_bias=not use_batchnorm,
            kernel_regularizer=kreg, name=base_name
        )(x)
        if use_batchnorm:
            x = layers.BatchNormalization(name=f"{base_name}_bn")(x)
            x = layers.Activation(act, name=f"{base_name}_act")(x)
        return x

    x = _conv2d_block(
        inp, conv1_filters, conv1_kernel, conv1_strides, conv1_padding_eff, conv1_act, base_name="conv1"
    )
    x = layers.MaxPooling2D(pool_size=pool1_size, strides=pool1_strides, name="pool1")(x)

    x = _conv2d_block(
        x, conv2_filters, conv2_kernel, conv2_strides, conv2_padding_eff, conv2_act, base_name="conv2"
    )
    x = layers.MaxPooling2D(pool_size=pool2_size, strides=pool2_strides, name="pool2")(x)

    # (3) GAP reduz drasticamente a dimensionalidade vs Flatten, tipicamente com baixo custo computacional [Lin13].
    if use_gap:
...
def make_tf_dataset(df: pd.DataFrame, cfg: Dict[str, Any], training: bool):
    ...
    def _load(path, y):
        ...
        if training:
            # Flip horizontal é barato e costuma ajudar em faces [Shorten19].
            if bool(cfg.get("AUG_HFLIP", True)):
                img = tf.image.random_flip_left_right(img)

            # Reflect pad + random crop (simula pequenas translações/zoom sem custo alto) [Shorten19].
            ...
            # Color jitter leve (sem JPEG-quality): brilho/contraste/saturação/hue [Shorten19].
            if bool(cfg.get("AUG_COLOR_JITTER", False)):
                br = float(cfg.get("AUG_BRIGHTNESS_DELTA", 0.0))
                if br and br > 0:
                    img = tf.image.random_brightness(img, max_delta=br)

                c_lo, c_hi = _as_range2(cfg.get("AUG_CONTRAST_RANGE", (1.0, 1.0)), "AUG_CONTRAST_RANGE")
                if (c_lo != 1.0 or c_hi != 1.0) and c_hi > c_lo:
                    img = tf.image.random_contrast(img, lower=c_lo, upper=c_hi)

                s_lo, s_hi = _as_range2(cfg.get("AUG_SATURATION_RANGE", (1.0, 1.0)), "AUG_SATURATION_RANGE")
                if (s_lo != 1.0 or s_hi != 1.0) and s_hi > s_lo:
                    img = tf.image.random_saturation(img, lower=s_lo, upper=s_hi)

                hue = float(cfg.get("AUG_HUE_DELTA", 0.0))
                if hue and hue > 0:
                    img = tf.image.random_hue(img, max_delta=hue)

                # Mantém em [0,1] antes da normalização (evita saturação numérica).
...
def make_loss(cfg: Dict[str, Any], num_classes: int):
    """
    Perda: Cross-Entropy (saída softmax), com opção de label smoothing.

    - Default (USE_SPARSE_CE=False): usa CategoricalCrossentropy (one-hot).
    - Opcional (USE_SPARSE_CE=True): usa SparseCategoricalCrossentropy (mais eficiente).
    - Opcional (LABEL_SMOOTHING>0): aplica label smoothing como regularização leve [Szegedy16].

    Observação prática:
    - Nem todas as versões de TF/Keras suportam label_smoothing em SparseCategoricalCrossentropy.
      Por isso há fallback seguro via one-hot + CategoricalCrossentropy.
    """
    import inspect

    use_sparse = bool(cfg.get("USE_SPARSE_CE", False))
    ls = float(cfg.get("LABEL_SMOOTHING", 0.0) or 0.0)
    ls = max(0.0, min(ls, 0.2))  # clamp conservador

    if use_sparse:
        if ls > 0:
            # Preferimos usar a implementação nativa se estiver disponível.
            try:
                if "label_smoothing" in inspect.signature(tf.keras.losses.SparseCategoricalCrossentropy).parameters:
                    return tf.keras.losses.SparseCategoricalCrossentropy(from_logits=False, label_smoothing=ls)
            except Exception:
                pass

            # Fallback: one-hot + CategoricalCrossentropy com label_smoothing
            cce = tf.keras.losses.CategoricalCrossentropy(from_logits=False, label_smoothing=ls)
    ...
    # Não-sparse (one-hot): podemos usar CategoricalCrossentropy direto
    return tf.keras.losses.CategoricalCrossentropy(from_logits=False, label_smoothing=ls)
...
def run_kfold_cv(cfg: Dict[str, Any]) -> Tuple[float, Path]:
    ...
    # --------------------------------------------------------
    # Persistir splits (pool do CV e holdout FINAL TEST) no run_dir
    # para reuso exato no script de avaliação (evita data leakage).
    # Observação: não altera a lógica do treino; apenas registra os
    # subconjuntos efetivamente usados.
    # --------------------------------------------------------
    try:
        cv_df.to_csv(run_dir / "cv_manifest.csv", index=False, encoding="utf-8")
        final_test_df.to_csv(run_dir / "final_test_manifest.csv", index=False, encoding="utf-8")
        save_json(
            {
                "final_test_class_fraction": float(cfg.get("FINAL_TEST_CLASS_FRACTION", 1.0)),
                "final_test_fraction_per_class": float(cfg.get("FINAL_TEST_FRACTION_PER_CLASS", 0.0)),
                "num_classes_full": int(df_full["y"].nunique()),
                "num_classes_cv_pool": int(cv_df["y"].nunique()) if len(cv_df) else 0,
                "num_classes_final_test": int(final_test_df["y"].nunique()) if len(final_test_df) else 0,
                "num_images_cv_pool": int(len(cv_df)),
                "num_images_final_test": int(len(final_test_df)),
                "final_test_classes_chosen": [int(c) for c in list(test_classes)],
            },
            run_dir / "holdout_meta.json",
        )
        print(f"[INFO] Saved leakage-free manifests: cv_manifest.csv, final_test_manifest.csv @ {run_dir}")
    except Exception as e:
        # Não quebrar o treino por causa de IO.
        print(f"[WARN] Não foi possível salvar manifests do holdout/CV: {e}")
...
#          Journal of Big Data.
# [Lin13]  Lin, M., Chen, Q., & Yan, S. (2013). Network In Network.
#          arXiv:1312.4400. (GAP como alternativa ao Flatten em CNNs).
# [Szegedy16] Szegedy, C. et al. (2016). Rethinking the Inception Architecture for Computer Vision.
#          arXiv:1512.00567. (Label smoothing).
