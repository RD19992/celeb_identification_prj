...
[ALTERAÇÕES IMPLEMENTADAS]
  1) [NOVO] Dropout na camada escondida (inverted dropout; só no treino)
  2) [NOVO] Early stopping (configurável) no CV e no treino final
  3) [NOVO] Prints: entropia (média/mediana), ||W1||, ||W2||, ||grad||, ||update|| por época
  4) [NOVO] Inicialização He/Xavier automática (ReLU -> He; tanh -> Xavier)
  5) [FIX] Robustez em confusion_matrix para evitar warning de "regression problem"
       (garante y_true/y_pred int64 1D; se y_pred vier 2D, aplica argmax)
"""
...
CONFIG = {
    ...
    # [NOVO] Dropout na camada escondida (prob. de "dropar")
    "dropout_hidden_p": 0.20,     # 0.0 desliga

    # [NOVO] Inicialização automática He/Xavier
    ...
    # [MELHORIA] Dropout + Armijo: line-search com dropout estocástico é ruim.
    # Aqui, por padrão, o Armijo é calculado SEM dropout (determinístico).
    ...
    # métrica: "val_loss" (minimiza) ou "val_acc" (maximiza)
    ...
    # [NOVO] Split interno treino/val no treino final (para early stopping)
    "final_val_frac": 0.10,   # 0 desliga val interno (logo, desliga early stop no final)
    "final_val_seed": 2026,
    ...
    # [NOVO] Stats por época (entropia, normas, etc.)
...
def _std_xavier(fan_in: int, fan_out: int) -> float:
    # Xavier normal (var = 2/(fan_in+fan_out)) -> std = sqrt(2/(fan_in+fan_out))
...
def inicializar_pesos(d: int, H: int, K: int, act_hidden: str, rng: np.random.Generator):
    ...
    [NOVO] Inicialização automática:
      - W1: ReLU -> He; tanh -> Xavier; softmax -> Xavier
      - W2: Xavier (por padrão)
    Bias = 0.
    """
    ...
    # W2: Xavier costuma ser estável para camada linear -> softmax
...
# ============================================================
# [NOVO] Dropout + utilitários de debug
# ============================================================

def aplicar_dropout_invertido(A: np.ndarray, p_drop: float, rng: np.random.Generator):
    """
    Inverted dropout: A_drop = A * mask, onde mask in {0, 1/(1-p)}.
    Retorna (A_drop, mask).
    """
    p = float(p_drop)
    if p <= 0.0:
        return A, None
    if p >= 0.999:
        # caso patológico: tudo dropa; evitamos
        return A, None
...
def entropia_stats(P: np.ndarray):
    """
    [NOVO] Entropia por amostra: -sum p log p.
    Retorna (mean, median).
    """
...
def mlp_forward(
    ...
    [NOVO] Dropout na camada escondida (somente se train_mode=True).
    """
...
def objective_total_mlp(
    ...
    # [MELHORIA] objective sempre em modo "eval" (sem dropout) para ser determinístico
    P, _ = mlp_forward(
        X, W1, b1, W2, b2,
...
def batch_grad_mlp_l2(
    ...
    [NOVO] Se train_mode=True, aplica dropout no forward e propaga máscara no backward.
    """
    B = int(X.shape[0])
    ...
    # backprop para A1 (após dropout)
    dA1 = dZ2 @ W2.T

    # [NOVO] aplica máscara de dropout (inverted dropout) antes da derivada da ativação
    ...
    if act_hidden_l == "softmax":
        dZ1 = softmax_backward(dA1, A1_pre)
    elif act_hidden_l == "tanh":
        dZ1 = tanh_backward(dA1, A1_pre)
    elif act_hidden_l == "relu":
        dZ1 = relu_backward(dA1, Z1)
    else:
        raise ValueError(f"Ativação desconhecida: {act_hidden_l}")
    ...
    return (
        ...
        P,  # [NOVO] útil para stats (entropia etc.)
    )
...
# ============================================================
# Treino SGD (MLP + L2) com Dropout + Early stopping + Debug stats
# ============================================================
...
def treinar_mlp_l2_sgd(
    ...
    # [NOVO] validação para early stopping
    ...
    # [NOVO] Early stopping bookkeeping
    ...
    for ep in range(int(epochs)):
        # ====================================================
        # 1) Armijo em probe batch (determinístico -> sem dropout)
        # ====================================================
        if use_armijo:
            ...
            # [MELHORIA] Para Armijo, por padrão não aplicamos dropout (mais estável)
            ...
            dW1_p, db1_p, dW2_p, db2_p, Pp = batch_grad_mlp_l2(
                ...
                rng=rng,
                train_mode=False,  # sem dropout/sem ruído
            )
            ...
            # [NOVO] Debug stats por época (probe)
            if bool(CONFIG["debug_epoch_stats"]):
                ...
                gb1 = norma_fro(db1_p)
                gb2 = norma_fro(db2_p)
                upd_W1 = float(alpha_ep) * gW1
                upd_W2 = float(alpha_ep) * gW2
                print(
                    f"[Armijo] ep={ep+1:03d}/{epochs} alpha={alpha_ep:.3e} bt={bt} "
                    f"F {F_old:.4f}->{F_new:.4f} | "
                    f"H(mean/med)={ent_m:.3f}/{ent_md:.3f} | "
                    f"||W1||={nW1:.2e} ||W2||={nW2:.2e} | "
                    f"||gW1||={gW1:.2e} ||gW2||={gW2:.2e} ||gb1||={gb1:.2e} ||gb2||={gb2:.2e} | "
                    f"||updW1||~{upd_W1:.2e} ||updW2||~{upd_W2:.2e}"
                )
            else:
        ...
        # ====================================================
        # 2) SGD mini-batches (aqui SIM usamos dropout)
        # ====================================================
        ...
        for bi, start in enumerate(range(0, m_total, batch_size), start=1):
            ...
            dW1, db1_g, dW2, db2_g, _ = batch_grad_mlp_l2(
                ...
                act_hidden=act_hidden, act_output=act_output,
                l2=float(l2), m_total=m_total,
                dropout_p=dropout_p,  # [NOVO] dropout no treino
                rng=rng,
                train_mode=True,
            )
            ...
            if bi % int(CONFIG["print_every_batches"]) == 0 or bi == n_batches:
                # loss determinístico (sem dropout) no próprio batch (estimativa rápida)
        ...
        # ====================================================
        # 4) Validação + Early stopping (opcional)
        # ====================================================
        if has_val:
            # val loss (sem dropout)
            ...
            print(f"[SGD] ep={ep+1:03d}/{epochs} alpha={alpha_ep:.3e} loss_sub~={loss_ep:.6f} | val_loss={val_loss:.6f} val_acc={val_acc:.4f}")
        else:
        ...
        if has_val and early_stop_enabled:
            ...
            if metric_name == "val_loss":
                metric_now = float(val_loss_hist[-1])
            else:
                metric_now = float(val_acc_hist[-1])
    ...
    # restaura melhor modelo (se habilitado)
    stopped_early = False
...
def predict_labels_mlp(X: np.ndarray, modelo: dict):
    ...
    P, _ = mlp_forward(
        ...
        dropout_p=0.0, rng=None, train_mode=False  # [MELHORIA] sempre sem dropout em predição
    )
...
def matriz_confusao_top_k(y_true: np.ndarray, y_pred: np.ndarray, top_k: int):
    """
    [FIX] Robustez para evitar warning do sklearn:
      - garante y_true/y_pred como vetores 1D int64
      - se y_pred vier como matriz de probabilidades (n, K), aplica argmax
    """
    top_k = int(top_k)
    if top_k <= 0:
        return
    ...
    if y_pred_arr.ndim == 2:
        # caso alguém tenha passado P por engano
    ...
    for l2 in grid_l2:
        ...
        for fold, (tr, va) in enumerate(skf.split(X_cv, y_cv), start=1):
            ...
            modelo = treinar_mlp_l2_sgd(
                ...
                use_armijo=True,
                # [NOVO] early stopping no CV: usa o próprio fold de validação
                X_val=Xva,
                y_val=yva,
...
# ============================================================
# [NOVO] Split interno treino/val para treino final (early stop)
# ============================================================
...
def main():
    ...
    # [NOVO] split interno treino/val para early stopping no final
    ...
    modelo = treinar_mlp_l2_sgd(
        ...
        use_armijo=True,
        # [NOVO] early stopping no treino final (se existir split interno)
        X_val=X_final_val,
        y_val=y_final_val,
    ...
    print("[Armijo FINAL] alpha_median=", st.get("alpha_median"),
